...
import re, io, base64, qrcode   # ← 產生並轉成 Base64 內嵌用
...
def qrcode_view():
    """每天固定一張 QR Code；隔日 00:00 自動失效。"""
    today  = date.today()
    s      = URLSafeTimedSerializer(current_app.config["SECRET_KEY"])
    token  = s.dumps({"day": today.isoformat()})           # 只簽日期
  ...
  <h3>今天 ({today}) 的打卡 QR Code</h3>
  <img src="data:image/png;base64,{b64}" alt="QR Code"><br>
  <small>{url}</small>
</body></html>""")
...
def form():
    ...
    if not session.get("token_ok"):
        ...
        if not token:
            ...
            s = URLSafeTimedSerializer(current_app.config["SECRET_KEY"])
            try:
                s.loads(token, max_age=86_400)            # ← 有效 24h
