...
def show_records():
    ...
    # 3. 決定顯示對象
    # 規則：若 eid 有值 → 單人；否則若 area 有值 → 該區所有員工；都沒選 → 空
    ...
    # 表單（變更 area 會清空 eid；選 eid 或 ym 直接提交）
    form_html = f"""
...
<script>
document.getElementById('areaSel').addEventListener('change', function() {{
  document.getElementById('eidSel').value = '';  // 選區域 → 先顯示整區，再可選單人
  document.getElementById('recForm').submit();
}});
document.getElementById('eidSel').addEventListener('change', function() {{
  document.getElementById('recForm').submit();
}});
document.getElementById('ymSel').addEventListener('change', function() {{
  document.getElementById('recForm').submit();
}});
</script>
"""
    ...
    for emp in targets:
        ...
        # - 單人模式：帶 eid 回來
        # - 區域模式：不帶 eid（維持整區列表）
        back = url_for("rec.show_records",
                       area=area,
                       eid=(emp.id if single_mode else ""),
                       ym=ym)
    ...
    if request.method == "POST":
        ...
        if typ == LEAVE_PTYPE:
            ...
            if rec:
                ...
                db.session.add(Checkin(employee_id=emp_id, work_date=dt, p_type=typ, ts=ts))
        db.session.commit()
        return redirect(back)
