...
def _rewrite_public_menu_xlsx(response):
    ...
    wb=Workbook();ws=wb.active;ws.title="公版菜單";ws.merge_cells("A1:Q1");ws["A1"]=f"{draft.name}　{draft.start_date.year}年{draft.start_date.month}月菜單";ws["A1"].font=Font(size=18,bold=True);ws["A1"].alignment=Alignment(horizontal="center",vertical="center");ws.row_dimensions[1].height=30
    headers=["日期","星期","主食","主菜","副菜","","蔬菜","湯品","全穀雜糧類(份)","豆魚蛋肉類(份)","蔬菜類(份)","油脂與堅果種子類(份)","水果類(份)","乳品類(份)","熱量(大卡)","營養狀態","規則分數"]
    ...
    for row in rows:
        ...
        rep=result.days[row["service_date"]];sv=rep.servings or {};status="；".join(rep.warnings or rep.ok_messages) or "—"
        top=[row["service_date"],row["weekday"]]+[(x.get("name","") if x else "") for x in dishes]+[sv.get("grains") or "",sv.get("protein") or "",sv.get("vegetable") or "",sv.get("oil") or "",sv.get("fruit") or "",sv.get("dairy") or "",row["kcal"] if row["kcal"] is not None else "",status,round(result.penalty/1000,1)]
        bottom=["",""]+[(x.get("ingredients","") if x else "") for x in dishes]+["", "", "", "", "", "", "", "", ""]
    ...
    widths={"A":12,"B":7,"C":21,"D":21,"E":19,"F":19,"G":17,"H":21,"I":10,"J":10,"K":9,"L":11,"M":8,"N":8,"O":11,"P":30,"Q":10}
    for col,w in widths.items():ws.column_dimensions[col].width=w
    ws.freeze_panes="C3";ws.sheet_view.showGridLines=False
    note=wb.create_sheet("說明");note.append(["欄位","說明"]);note.append(["六大類份數","依 Recipe BOM 每人用量估算；屬排菜用估值，不是營養師簽證值。"]);note.append(["可匯回總表","保留日期、星期、主食、主菜、副菜、蔬菜、湯品欄位；第二列食材與右側份數欄會被既有總表匯入器忽略。"]);note.column_dimensions["A"].width=20;note.column_dimensions["B"].width=90
